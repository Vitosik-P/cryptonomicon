    ...
    <div class="container">
      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div
          ...
          :key="t.name"
        >
          <div class="px-4 py-5 sm:p-6 text-center" @click="select(t)">
      ...
      <section class="relative" v-if="sel">
        ...
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div
            v-for="(g, idx) in normalaizeGraph()"
            :style="{ height: `${g}%` }"
            :key="idx"
            class="bg-purple-800 border w-10"
          ></div>
        </div>
        data {{ JSON.stringify(normalaizeGraph()) }}
        <button
          type="button"
...
export default {
  ...
  methods: {
    add() {
      ...
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/pricemulti?fsyms=${currentTicker.name}&tsyms=USD&api_key=cfa95584bd62ef1eb869fae70f505ecb13341a86ce18c71fd3dac03aeb986f49`
        );
        ...
        if (this.sel?.name === currentTicker.name) {
          this.graph.push(data[`${currentTicker.name}`].USD);
        }
      }, 1000);
      this.ticker = "";
    },
